...
- The parent Codex task owns goals, workstreams, capacity decisions, owner/model assignments, dependencies, risks, intervention, compact evidence review, and reporting. It defaults to adaptive manager-and-integrator mode and may own the critical path directly. Strict manager-only behavior applies only when the user explicitly requests it.
...
- Continuous execution alone never authorizes a heartbeat. Create one only for an explicit periodic-report request, reuse the same thread, use a 30-240 minute cadence, call status with `waitSeconds=0`, and pause it whenever no worker is active or the run is blocked, decision-ready, or terminal. `Use no plugin` and equivalent stop wording pauses it before any MCP call.
...
- Adaptive mode takes back a bounded critical-path item after one failed or no-change delegation. Strict manager-only mode may rescope once, then must report the delivery blocker and offer adaptive takeover instead of cycling workers.
- Verify cheaply: deterministic gates first, then one compact parent-Codex review. Never spend a second premium model to re-evaluate successful Fable/Opus/Sonnet work. A genuinely necessary independent reasoning review must use a cheaper capable model, receive only the bounded diff and gates, and target no more than 10% of the producer's output.
...
| "Codex fixed it after the worker failed" | In adaptive mode, record the bounded takeover and evidence after one failed delegation; strict manager-only mode must ask before switching modes |
