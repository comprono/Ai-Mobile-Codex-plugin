...
The runtime rejects semantic overlap, path overlap, and duplicate active lanes before dispatch. The result should contain only the achieved outcome, changed files, checks, and one concrete blocker. Read `detail=compact` once at integration, using `waitSeconds=60` to absorb a short remaining delay inside the bridge, and use the result before taking over the worker lane. Use `detail=full` only to diagnose a real failure.
