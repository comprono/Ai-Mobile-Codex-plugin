...
- Treat the current Codex task and optional Codex workers as different resources. Current Codex owns the critical path and integration by default; an explicitly manager-only parent still permits separate Codex execution. Prefer durable standalone CLI jobs when ChatGPT-plan auth is verified; otherwise selected host-native workers execute through the host spawn tool.
