...
try {
  if (action === "serve") serve(entrypoint);
    ...
    const input = jsonInput(); const workspace = safeWorkspace(input.workspace); const resources = inventory({}); const decision = route({ ...input, workspace }, resources);
    output(decision.action === "delegate" ? { dispatched: true, ...createJob({ ...decision.request, workspace, provider: decision.provider, providerCommand: resources.providers[decision.provider].command }, entrypoint) } : { dispatched: false, action: "current-codex", reason: decision.reason });
  } else if (action === "read-job-cli") output(readJob(arg("--workspace"), arg("--job-id"), arg("--detail", "compact")));
