...
const TOOLS = [
  ...
  } } },  { name: "run-task-cycle", description: "Run one bounded deterministic task cycle without an LLM manager loop: dispatch dependency-ready work, wait for finite workers, collect terminal results once, integrate verified patches, and continue only while acceptance progress or a materially changed recovery path exists.", inputSchema: { type: "object", required: ["taskId"], properties: { taskId: { type: "string" }, maxRounds: { type: "number", minimum: 1, maximum: 5 }, maxMinutes: { type: "number", minimum: 1, maximum: 30 }, noProgressLimit: { type: "number", minimum: 1, maximum: 3 }, horizonHours: { type: "number", minimum: 1, maximum: 24 } } } },
