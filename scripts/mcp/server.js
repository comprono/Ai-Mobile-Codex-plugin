...
const TOOLS = [
  { name: "start-task", description: "First AI Mobile call for one project or a multi-project portfolio. Recovers bounded intent and executable blockers, captures capacity, and returns a same-turn current-Codex action plus model-use reasons. Codex must start that action before replying when mustStartNow is true. It starts no UI, loop, Goal, automation, or heartbeat.", inputSchema: { type: "object", anyOf: [{ required: ["workspace"] }, { required: ["projects"] }], properties: {
