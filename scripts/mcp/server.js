...
const TOOLS = [
  ...
  } } },  { name: "run-task-cycle", description: "Run one bounded deterministic task cycle without an LLM manager loop or desktop launch. Each call stays below the MCP host deadline and returns continuationRequired when the same finite worker still owns the action.", inputSchema: { type: "object", required: ["taskId"], properties: { taskId: { type: "string" }, maxRounds: { type: "number", minimum: 1, maximum: 5 }, maxMinutes: { type: "number", minimum: 1, maximum: 30 }, sliceSeconds: { type: "number", minimum: 30, maximum: 240 }, noProgressLimit: { type: "number", minimum: 1, maximum: 3 }, horizonHours: { type: "number", minimum: 1, maximum: 24 } } } },
