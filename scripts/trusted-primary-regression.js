...
assert.equal(handoff.expectedRuntimeVersion, "1.2.2");
...
assert.match(handoff.resumePrompt, /Invoke run-task-cycle for this task/i);
assert.match(handoff.resumePrompt, /maxRounds 3, maxMinutes 15, sliceSeconds 210, noProgressLimit 2/i);
assert.match(handoff.resumePrompt, /continuationRequired true/i);
...
assert.equal(dry.ExpectedRuntimeVersion, "1.2.2");
