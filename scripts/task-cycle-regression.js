...
fs.writeFileSync(path.join(fakeBin, "fake-claude-read.js"), 'process.stdout.write(JSON.stringify({is_error:false,model:"sonnet",structured_output:{outcome:"Found one exact bounded implementation.",evidence:["verify.js exists"],checks:["node verify-inspection.js"],blocker:"",blockerOwner:"",recoveryTrigger:"",recoveryAction:"",proposedWorkUnits:[{goal:"Create inspection.txt containing INSPECTED_OK",relevantFiles:["verify.js","verify-inspection.js","inspection.txt"],expectedFiles:["inspection.txt"],acceptanceCriteria:["node verify-inspection.js exits zero"],verificationCommands:[{name:"verify-inspection",command:"node",args:["verify-inspection.js"],timeoutSeconds:30}],taskKind:"code",complexity:"medium",priority:100,requiredCapabilities:["source","local-files","tests"]}]},usage:{input_tokens:30,cache_creation_input_tokens:0,cache_read_input_tokens:0,output_tokens:12}}));\n', "utf8");
...
const invalidObservationOutput = JSON.stringify({ is_error: false, model: "sonnet", structured_output: invalidObservationPlan, usage: { input_tokens: 24, cache_creation_input_tokens: 0, cache_read_input_tokens: 0, output_tokens: 10 } });
...
writeProfile({ minimumFreeRamMb: 128, worktreeMinFreeMb: 1 });
