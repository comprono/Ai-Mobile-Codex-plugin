...
function prepareIsolatedWorkspace(workspaceValue, taskId, jobId, readOnly, profileValue) {
  ...
  const resolvedGitRoot = fs.existsSync(gitRoot) ? fs.realpathSync(gitRoot) : gitRoot;
  const resolvedWorkspace = fs.existsSync(workspace) ? fs.realpathSync(workspace) : workspace;
