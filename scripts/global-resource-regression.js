...
try {
  ...
  const contract = (provider, pools, fairnessKey) => ({ taskId: "task-fixture-0001", provider, quotaPoolIds: pools, fairnessKey, workspace: workspaceRoot, timeoutSeconds: 60 });
  ...
  const firstTask = createTaskRecord({ workspace: workspaceRoot, outcome: "First project", requirements: [] });
  const secondTask = createTaskRecord({ workspace: workspaceRoot, outcome: "Second project", requirements: [] });
  const first = createJob({
    taskId: firstTask.taskId, workspace: workspaceRoot, goal: "Edit shared ownership file for first project", projectGoal: "First project", currentCodexGoal: "Inspect another file", independenceReason: "Fixture",
  ...
  assert.throws(() => createJob({
    taskId: secondTask.taskId, workspace: workspaceRoot, goal: "Edit shared ownership file for second project", projectGoal: "Second project", currentCodexGoal: "Inspect another file", independenceReason: "Fixture",
