...
The one-line form is sufficient only when the same task already has an active Goal or AI Mobile run with `RootGoal`. This means: keep this existing Codex task as the only user-facing control room; do not create another Codex task, thread, or Goal, and never a detached chat; do create and manage separate native Codex workers and headless Claude/Antigravity/Cursor jobs when eligible. An explicit request for continuous, proactive, unattended, or 24/7 management additionally authorizes exactly one reporting heartbeat attached to this same thread (reused when one already matches) that resumes the same Goal and run, reports transitions first with a concise checkpoint when unchanged, and is disabled or removed when the user stops continuous management.
...
- **Writer boundaries:** up to three writers may run together when their workspace-relative file or directory boundaries are explicit and pairwise disjoint. Overlapping or unscoped writers remain serialized.
